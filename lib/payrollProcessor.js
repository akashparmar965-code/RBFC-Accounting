...
export function allocatePayrollByHours(companyPayrollRows, storeMaster, storeHours) {
  ...
  // Sort each company's stores so which one absorbs the rounding remainder
  // (below) is deterministic and reproducible.
  for (const company of Object.keys(companyStores)) {
    companyStores[company].sort((a, b) => a.localeCompare(b));
  }
  ...
  for (const payrollRow of companyPayrollRows) {
    ...
    const weights = stores.map((store) => Number(storeHours[store] || 0) / totalCompanyHours);
    const rowsForCompany = stores.map((store) => ({ company, store, storeLabel: storeOutputName[store] || store }));

    // Round each store's share independently, except the last store (by the
    // sort above) gets the remainder — company amount minus everyone else's
    // rounded share — so the stores' totals always sum exactly back to what
    // was typed in, instead of drifting a cent or two from independent
    // per-store rounding.
    for (const { key } of PAYROLL_FIELDS) {
      const companyAmount = toNumber(payrollRow[key]);
      let allocatedSoFar = 0;
      stores.forEach((store, idx) => {
        if (idx === stores.length - 1) {
          rowsForCompany[idx][key] = round2(companyAmount - allocatedSoFar);
        } else {
          const rounded = round2(companyAmount * weights[idx]);
          rowsForCompany[idx][key] = rounded;
          allocatedSoFar += rounded;
        }
      });
    }

    allocatedRows.push(...rowsForCompany);
  }
