 ...
 *
 * Also looks for a "Time Period: <Month> <Year>: (MM/DD/YYYY - MM/DD/YYYY)"
 * line in the preamble (these reports always seem to carry one) and
 * returns its start/end dates as "MM/DD/YYYY" strings, so the caller can
 * check the file actually covers the pay period the user selected instead
 * of trusting the file name alone.
 *
 * @param {ArrayBuffer} arrayBuffer
 * @returns {{ values: Object<string, number>, unmatchedHeaders: string[], periodStart: string|null, periodEnd: string|null }}
 ...
 *   silently get dropped). periodStart/periodEnd are null if no "Time
 *   Period: ..." line was found.
 */
export function parsePayrollReportFile(arrayBuffer) {
  ...
  outer: for (const row of rows) {
    for (const cell of row || []) {
      ...
      const match = String(cell).match(
        /Time Period:.*\(\s*(\d{1,2}\/\d{1,2}\/\d{4})\s*-\s*(\d{1,2}\/\d{1,2}\/\d{4})\s*\)/i
      );
      if (match) {
        periodStart = match[1];
        periodEnd = match[2];
  ...
  return { values, unmatchedHeaders, periodStart, periodEnd };
}
...
/**
 * Employee Timesheet exports don't carry a "Time Period" line inside the
 * sheet like the payroll Custom Reports do — the date range is only in
 * the file name instead (e.g.
 * "Employee_time_Sheet-Payroll_01012026-01312026 (2).xlsx", two 8-digit
 * MMDDYYYY blocks joined by a hyphen). Returns "MM/DD/YYYY" strings, or
 * both null if the name doesn't contain that pattern (nothing to check
 * against in that case — never guessed).
 *
 * @param {string} fileName
