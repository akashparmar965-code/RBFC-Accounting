...
// Same pseudo-store Store Transfer already recognizes (`WAREHOUSE_NAME` in
// storeTransferProcessor.js) — a real sender/receiver in these exports
// that was never a Store Master row by design. Matched case-insensitively,
// same convention.
export const WAREHOUSE_NAME = "Company Warehouse";

/**
 * Remap each snapshot row's Store through the shared rename fallback
 * (`store_name_mappings`), then keep only rows matching a real Store
 * Master store (Active or Closed — both are real physical locations that
 * can hold real stock; status isn't what determines relevance here) or
 * the recognized `WAREHOUSE_NAME` pseudo-store — the same "match Store
 * Master, flag/exclude unmatched, never guess" convention every other
 * upload page in this app already follows, which this page never had.
 * Confirmed live on a real Opening file: rows with a blank Store totaled
 * $1.56M and 21 further store name variants with no Store Master match at
 * all (renames/typos, none covered by `store_name_mappings`) totaled
 * ~$52K more — both silently inflating Opening/Closing Value, and able to
 * corrupt Missing/Store Mismatch too, since those rely on Opening/
 * Closing's own Store field as ground truth for `expectedStore`/
 * `closeEvent.store`. (A stricter "exclude Closed-status stores" version
 * was tried first — real data showed the user's own hand-filtered file
 * excluded 2 Closed stores worth $31K but *kept* 4 other Closed stores
 * worth $11K, so Closed-status isn't actually the rule; reverted rather
 * than guess an ad hoc dollar threshold.)
 *
 * @param {Array} rows - parsed snapshot rows (Opening or Closing)
 * @param {Array} storeMaster - rows from the `stores` table
 * @param {Object} storeNameMap - output of buildStoreNameMap()
 * @returns {{ rows: Array, unmatchedStores: string[] }} rows = only the
 *   matched subset (Store remapped to its canonical Elevate Name, or left
 *   as `WAREHOUSE_NAME`); unmatchedStores = distinct raw values with no
 *   Store Master match at all, `"(blank)"` for a missing Store field, for
 *   flagging via savePendingMappings.
 */
export function matchSnapshotRowsToStores(rows, storeMaster, storeNameMap = {}) {
  const knownStores = new Set();
  for (const s of storeMaster || []) {
    const name = s.elevate_name ? String(s.elevate_name).trim() : "";
    if (name) knownStores.add(name);
  }

  const matched = [];
  const unmatchedSet = new Set();
  for (const row of rows) {
    const raw = row["Store"] != null ? String(row["Store"]).trim() : "";
    if (!raw) {
      unmatchedSet.add("(blank)");
      continue;
    }
    if (raw.toLowerCase() === WAREHOUSE_NAME.toLowerCase()) {
      matched.push(raw === WAREHOUSE_NAME ? row : { ...row, Store: WAREHOUSE_NAME });
      continue;
    }
    const mapped = storeNameMap[raw] || raw;
    if (!knownStores.has(mapped)) {
      unmatchedSet.add(raw);
      continue;
    }
    matched.push(mapped === raw ? row : { ...row, Store: mapped });
  }

  return { rows: matched, unmatchedStores: Array.from(unmatchedSet) };
}
