// Carries unmatched-door/unmapped-product/unmatched-account results from a
// Bills/JV Entry upload over to the Mapping Master page, so they don't
// vanish when you navigate away to go fix them. Lives in sessionStorage
// (survives page navigation within the tab) and is explicitly cleared on
// sign-out.
const KEY = "rbfc_pending_mappings";

const EMPTY = { unmatchedDoors: [], unmappedProducts: [], unmatchedAccounts: [] };
...
/** Merge new unmatched doors / unmapped products / unmatched accounts in from a fresh upload. */
export function savePendingMappings({ unmatchedDoors = [], unmappedProducts = [], unmatchedAccounts = [] }) {
  if (!isBrowser()) return;
