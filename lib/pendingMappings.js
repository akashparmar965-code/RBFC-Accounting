// Carries unmatched-door/unmapped-product results from a Bills/JV Entry
// upload over to the Mapping Master page, so they don't vanish when you
// navigate away to go fix them. Lives in sessionStorage (survives page
// navigation within the tab) and is explicitly cleared on sign-out.
...
export function loadPendingMappings() {
  if (!isBrowser()) return { unmatchedDoors: [], unmappedProducts: [] };
  try {
    const raw = window.sessionStorage.getItem(KEY);
    if (!raw) return { unmatchedDoors: [], unmappedProducts: [] };
    ...
    return { unmatchedDoors: [], unmappedProducts: [] };
  }
}
...
/** Merge new unmatched doors / unmapped products in from a fresh upload. */
export function savePendingMappings({ unmatchedDoors = [], unmappedProducts = [] }) {
  if (!isBrowser()) return;
  ...
  save({ unmatchedDoors: doors, unmappedProducts: Array.from(productMap.values()) });
}
