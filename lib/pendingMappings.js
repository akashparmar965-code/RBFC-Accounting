...
export function savePendingMappings({
  ...
  const incentiveProductMap = new Map(existing.unmappedIncentiveProducts.map((p) => [productKey(p), p]));
  for (const p of unmappedIncentiveProducts) incentiveProductMap.set(productKey(p), p);
...
/** Same as removePendingProductsMatching, for Incentives' own pending list. */
