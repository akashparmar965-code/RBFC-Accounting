  ...
  try {
    ...
    return {
      ...
      unmappedCreditNoteProducts: Array.isArray(parsed.unmappedCreditNoteProducts)
        ...
        : [],
      unmappedIncentiveProducts: Array.isArray(parsed.unmappedIncentiveProducts)
        ? parsed.unmappedIncentiveProducts
        : [],
    };
  } catch {
    return { ...EMPTY };
...
export function savePendingMappings({
  ...
  // Incentive items have no doorNumber/invoiceNo/product shape (the Credit
  // Memo export has no Door Number at all) -- keyed on creditMemoNumber+memo
  // instead of the shared productKey above.
  const incentiveKey = (p) => `${p.creditMemoNumber}||${p.memo}`;
  const incentiveProductMap = new Map(existing.unmappedIncentiveProducts.map((p) => [incentiveKey(p), p]));
  for (const p of unmappedIncentiveProducts) incentiveProductMap.set(incentiveKey(p), p);
  ...
  save({
    ...
    unmappedIncentiveProducts: Array.from(incentiveProductMap.values()),
  });
}
...
/** Same as removePendingProductsMatching, for Incentives' own pending list -- keyed on `memo`, not `product`. */
export function removePendingIncentiveProductsMatching(prefix, matchType = "starts_with") {
  if (!isBrowser()) return;
  const existing = loadPendingMappings();
  const lower = prefix.toLowerCase();
  save({
    ...existing,
    unmappedIncentiveProducts: existing.unmappedIncentiveProducts.filter(
      (p) => !textMatchesPrefix(p.memo.toLowerCase(), lower, matchType)
    ),
  });
}
