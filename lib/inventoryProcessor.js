...
/** Parse the uploaded Stock Trigger File (xlsx) and return its "Opening" and "Closing" sheets as row objects. */
export function parseStockTriggerWorkbook(arrayBuffer) {
  const wb = XLSX.read(arrayBuffer, { type: "array", cellDates: true });
  const findSheet = (name) => wb.SheetNames.find((n) => n.trim().toLowerCase() === name);
  const openingName = findSheet("opening");
  const closingName = findSheet("closing");
  if (!openingName || !closingName) {
    throw new Error('This file doesn\'t look like a Stock Trigger File — expected an "Opening" sheet and a "Closing" sheet.');
  }
  const openingRows = XLSX.utils.sheet_to_json(wb.Sheets[openingName], { defval: null });
  const closingRows = XLSX.utils.sheet_to_json(wb.Sheets[closingName], { defval: null });
  return { openingRows, closingRows };
}
