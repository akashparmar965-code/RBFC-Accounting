 ...
 * Credit Note's Products text is far messier than Bills' (one-off text
 * like "Loans and Exchange - transfer commission withhold earned on
 * 4/16/26 due to OC New Age 4/1/26"), so "contains" matching on a stable
 * prefix like "Loans and Exchange" is expected to do a lot of the work
 * here.
 */
...
/**
 * Classify a Credit Note-sheet line by its Products text, against
 * credit_note_mappings — a dedicated mapping table, deliberately separate
 * from Bills' product_mappings, since Credit Note's Products values are a
 * completely different vocabulary (ROI Account, Dealer Weekly Incentive
 * Credit, RMA Shipping Fee, Dealer Chargeback, CFC Violation, ad-hoc
 * Loans and Exchange text, even the occasional store name) rather than
 * device/accessory SKUs. A line matching no mapped rule is "unmapped".
 */
function classifyCreditNoteProduct(productRaw, memoRaw, creditNoteMappings) {
  const productLower = productRaw.toLowerCase();
  const rule = creditNoteMappings.find((m) => productMatchesRule(productLower, m));
 ...
 * resolved Expense Account) — mirrors billsProcessor.js's
 * aggregateBillLines exactly, including the same Other Services VIP /
 * Deductions split (Other Cost + Shipping Cost, and −(Discount + Other
 * Deductions), both invoice-level values captured once per invoice, never
 * summed across its lines — confirmed the raw export repeats them
 * identically per line here too, same as the Bill sheet). Per explicit
 * instruction, Credit Note line amounts are NOT sign-flipped — every
 * Product Total in the real files is already a positive credit magnitude,
 ...
 * unchanged, per explicit confirmation to keep the split identical).
 *
 * @param {Array} rawRows
...
export function aggregateCreditNoteLines(rawRows, creditNoteMappings) {
  ...
  for (const row of rawRows) {
    ...
    const productRaw = row["Products"] != null ? String(row["Products"]).trim() : "";
    const classified = classifyCreditNoteProduct(productRaw, memoRaw, creditNoteMappings);
    const key =
    ...
    if (!groups[key]) {
      groups[key] = {
        ...
        amount: 0,
        memo: classified.expenseMemo || productRaw,
        product: productRaw,
      };
    }
