...
export function aggregateBillLines(rawRows, productMappings) {
  ...
  for (const row of rawRows) {
    ...
    // New format dropped the Memo column entirely — fall back to the
    // product/line-item text (Name) instead, per explicit confirmation.
    const memoRaw = isNew ? "" : row["Memo"] != null ? String(row["Memo"]).trim() : "";
    const classified = classifyProduct(productRaw, isNew ? productRaw : memoRaw, productMappings);
    const key =
