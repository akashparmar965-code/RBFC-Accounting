 ...
 * Mapping Master page), prefix-matched case-insensitively. A line whose
 * Products doesn't start with any mapped prefix is "unmapped".
 */
function classifyProduct(productRaw, memoRaw, productMappings) {
  const productLower = productRaw.toLowerCase();
  const rule = productMappings.find((m) => productLower.startsWith(String(m.product_prefix).toLowerCase()));
