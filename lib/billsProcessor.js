 ...
 */
function classifyProduct(productRaw, memoRaw, productMappings) {
 ...
 *
 * Also emits one extra "Other Charges" line per invoice = Other Cost +
 * Shipping Cost − Discount − Other Deductions, posted to
 * OTHER_CHARGES_ACCOUNT. Those 4 columns are invoice-level values the raw
 * export repeats identically on every product line of that invoice (not a
 * per-line additive amount) — captured once per invoice, never summed
 * across its lines. Always emitted, even when the net is exactly 0.
 *
 * @param {Array} rawRows
...
export function aggregateBillLines(rawRows, productMappings) {
  ...
  for (const row of rawRows) {
    ...
    const classified = classifyProduct(productRaw, memoRaw, productMappings);
    const key =
  ...
  for (const inv of Object.values(invoiceOtherCharges)) {
    const key = `${inv.doorNumber}||${inv.invoiceNo}||otherCharges`;
    groups[key] = {
      ...
      date: inv.date,
      amount: inv.otherCost + inv.shippingCost - inv.discount - inv.otherDeductions,
...
export function buildBillRows(groupedLines, storeMaster, category = "all", doorMappings = []) {
  ...
  for (const line of groupedLines) {
    // Other Charges lines always post, even at $0 net, so every invoice's
    // charge (or lack of one) is visible rather than silently dropped.
    if (line.amount === 0 && !line.isOtherCharges) continue;
