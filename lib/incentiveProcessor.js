...
/**
 * Parse the uploaded VIP export (xlsx) and return rows from the same
 * "Credit Note" sheet the Credit Note tab reads — there is no separate
 * "Incentives" sheet in the VIP export. Incentives is a second, parallel
 * classification of that same raw data against its own incentive_mappings
 * table (e.g. "Weekly Incentive Credit" memos), not a different source
 * sheet. Both old and new column shapes are supported, auto-detected per
 * file, same as creditNoteProcessor.js's parseVipCreditNoteWorkbook.
