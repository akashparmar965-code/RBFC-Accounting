...
// The store that absorbs each company's rounding remainder in the Company
// Split tab, so per-store shares always sum back to the entered amount
// exactly instead of drifting by a cent or two when it doesn't divide
// evenly. Chosen by the user per company, not derived (e.g. not simply
// "alphabetically last") — extend this map if another company needs one.
export const COMPANY_ROUNDING_STORE = {
  "RBFC CLEVELAND LLC": "CL - Parma",
  "RBFC COLUMBUS LLC": "CB - Westerville",
  "RBFC NC LLC": "SC - East Town",
};
 ...
 *     If the company has a COMPANY_ROUNDING_STORE entry and more than one
 *     Active store, every other store gets the plain round2 share and that
 *     one designated store gets amount minus the others' total — absorbing
 *     the rounding remainder instead of leaving the JE out of balance.
 ...
 *   roundingAdjustments = for companies with a COMPANY_ROUNDING_STORE entry
 *   that actually got a split_per_store line, how much extra (or less) than
 *   a plain fair share was placed in that store, summed across all of that
 *   company's lines.
 */
export function buildCompanySplitJournalEntryRows(lines, byCompany, journalDate) {
  ...
  for (const line of lines) {
    ...
    if (line.split_per_store) {
      const roundingStoreName = COMPANY_ROUNDING_STORE[line.company_name];
      const roundingStore = roundingStoreName ? stores.find((s) => s.store === roundingStoreName) : null;

      if (roundingStore && stores.length > 1) {
        const fairShare = round2(amount / stores.length);
        const otherStores = stores.filter((s) => s.store !== roundingStore.store);
