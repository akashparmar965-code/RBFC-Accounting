...
/**
 * "Company Warehouse" is a real sender/receiver in the source file's
 * From/To columns but isn't (and never will be) a Store Master row — it
 * has no company books of its own. Treated as a special known "store"
 * whose company is itself (`WAREHOUSE_NAME`), matched case-insensitively
 * trimmed, so a Warehouse-involved row isn't dropped as unmatched: the
 * real store's side still posts normally, with the Warehouse showing up
 * only as that store's Stock Transfer counterpart label ("Stock
 * Transfer: Company Warehouse") — see buildStoreTransferJournalEntryRows,
 * which skips ever generating a JE section *for* the Warehouse itself.
 */
export const WAREHOUSE_NAME = "Company Warehouse";

/**
 * Match each transfer row's "From"/"To" store to Store Master (exact
 * `elevate_name`, no fallback table — same convention as Payroll/
 * Inventory) or to the special WAREHOUSE_NAME pseudo-store. A row is
 * dropped (and both raw names flagged) unless BOTH sides match, since a
 * one-sided entry can't be classified same-company vs inter-company or
 * given a safe Class.
 *
...
export function buildTransferRows(rawRows, storeMaster) {
  ...
  const resolve = (name) => {
    if (Object.prototype.hasOwnProperty.call(companyByStore, name)) {
      return { known: true, company: companyByStore[name], label: qboByStore[name] };
    }
    if (name.toLowerCase() === WAREHOUSE_NAME.toLowerCase()) {
      return { known: true, company: WAREHOUSE_NAME, label: WAREHOUSE_NAME };
    }
    return { known: false, company: "", label: "" };
  };
  ...
  for (const row of rawRows) {
    ...
    const fromInfo = resolve(from);
    const toInfo = resolve(to);
    if (!fromInfo.known) unmatchedSet.add(from);
    if (!toInfo.known) unmatchedSet.add(to);
    if (!fromInfo.known || !toInfo.known) continue;

    transferRows.push({
      from,
      fromCompany: fromInfo.company,
      fromLabel: fromInfo.label,
      to,
      toCompany: toInfo.company,
...
/**
 * Default account names, used whenever the caller doesn't pass (or is
 * missing a key from) `accountNames` — matches Mapping Master's
 * `stock_transfer_account_names` table's seeded defaults, so this stays
 * correct even before that table has loaded.
 */
export const DEFAULT_STOCK_TRANSFER_ACCOUNT_NAMES = {
...
/**
 * Stock Transfer JE, grouped by company. For every store, its transfers
 * are grouped by counterpart company: same-company transfers combine into
 * one "Stock Transfer" line, each different company gets its own
 * "Stock Transfer: <Company>" line (so inter-company balances are
 * trackable; intra-company transfers don't touch the balance sheet). The
 * natural movement account (Devices Transfer Out for the sending store,
 * Devices Transfer In for the receiving store) stays as ONE combined line
 * per store — only the Stock Transfer side is split by company.
 *
 *   Out (sender S, own company C_S): Cr Devices Transfer Out [store total];
 *     per destination-company group: Dr "Stock Transfer[: <company>]" [group total]
 *   In (receiver T, own company C_T): Dr Devices Transfer In [store total];
 *     per source-company group: Cr "Stock Transfer[: <company>]" [group total]
 *
 ...
 * company's JE.
 *
 * The 3 account names are editable on Mapping Master's "Stock Transfer"
 * tab (`stock_transfer_account_names` table) rather than hardcoded — pass
 * that table's rows (mapped to { devices_transfer_out, devices_transfer_in,
 * stock_transfer }) as `accountNames`; any missing/blank key falls back to
 * DEFAULT_STOCK_TRANSFER_ACCOUNT_NAMES.
 *
 ...
 * @param {Object} [accountNames] - { devices_transfer_out, devices_transfer_in, stock_transfer }
 * @returns {Object<string, Array>} byCompany
 */
export function buildStoreTransferJournalEntryRows(transferRows, journalDate, accountNames = {}) {
  const names = { ...DEFAULT_STOCK_TRANSFER_ACCOUNT_NAMES, ...accountNames };
  const devicesOutName = names.devices_transfer_out?.trim() || DEFAULT_STOCK_TRANSFER_ACCOUNT_NAMES.devices_transfer_out;
  const devicesInName = names.devices_transfer_in?.trim() || DEFAULT_STOCK_TRANSFER_ACCOUNT_NAMES.devices_transfer_in;
  const stockTransferName = names.stock_transfer?.trim() || DEFAULT_STOCK_TRANSFER_ACCOUNT_NAMES.stock_transfer;
  ...
  for (const g of Object.values(outGroups)) {
    // The Warehouse isn't a real company with its own books — it only ever
    // appears as the *other* store's Stock Transfer counterpart label.
    if (g.company === WAREHOUSE_NAME) continue;
    if (g.total) addLine(g.company, { Date: journalDate, Account: devicesOutName, Debit: "", Credit: g.total, Class: g.label });
    for (const [destCompany, amount] of Object.entries(g.byDestCompany)) {
      if (!amount) continue;
      const account = destCompany ? `${stockTransferName}: ${destCompany}` : stockTransferName;
  ...
  for (const g of Object.values(inGroups)) {
    if (g.company === WAREHOUSE_NAME) continue;
    if (g.total) addLine(g.company, { Date: journalDate, Account: devicesInName, Debit: g.total, Credit: "", Class: g.label });
    for (const [sourceCompany, amount] of Object.entries(g.bySourceCompany)) {
      if (!amount) continue;
      const account = sourceCompany ? `${stockTransferName}: ${sourceCompany}` : stockTransferName;
...
export function rowsToXlsxBuffer(rows, sheetName = "Stock Transfer JE") {
