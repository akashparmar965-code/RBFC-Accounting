...
export function buildStoreTransferJournalEntryRows(transferRows, journalDate) {
  ...
  for (const g of Object.values(outGroups)) {
    if (g.total) addLine(g.company, { Date: journalDate, Account: "Devices Transfer Out", Debit: "", Credit: g.total, Class: g.label });
    for (const [destCompany, amount] of Object.entries(g.byDestCompany)) {
      if (!amount) continue;
      const account = destCompany ? `Stock Transfer: ${destCompany}` : "Stock Transfer";
  ...
  for (const g of Object.values(inGroups)) {
    if (g.total) addLine(g.company, { Date: journalDate, Account: "Devices Transfer In", Debit: g.total, Credit: "", Class: g.label });
    for (const [sourceCompany, amount] of Object.entries(g.bySourceCompany)) {
      if (!amount) continue;
      const account = sourceCompany ? `Stock Transfer: ${sourceCompany}` : "Stock Transfer";
